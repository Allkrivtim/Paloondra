...
export default function AuditLogPanel() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(getErrorMessage(err, t('auditLog.failedToLoad')));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  return (
    <div className="rounded-xl border border-panel-border bg-panel-surface p-4">
      <h2 className="mb-3 text-sm font-semibold text-panel-text">{t('auditLog.title')}</h2>
      {loading && (
      ...
      {!loading && !error && entries.length === 0 && (
        <p className="text-sm text-panel-muted">{t('auditLog.noActions')}</p>
      )}
      {!loading && !error && entries.length > 0 && (
